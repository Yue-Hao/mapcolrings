...
(defvar exit_flag) ;; a global flat to exit the searching when get one result
...
(defun gen-tree-colorings-aux-not-used (t_coloring t_sorted domains c_coloring states)
  ...
  (let ((n (first t_sorted)))
	...
	(let ((parent_color (second (assoc (second n) t_coloring))))
	  (dolist (x (remove parent_color (second (assoc (first n) domains))))
	    (gen-tree-colorings-aux-not-used (append t_coloring (list (list (first n) x)))
				    (rest t_sorted)
				    domains
				    c_coloring
				    states))))))
...
(defun gen-tree-colorings-aux (t_coloring t_sorted domains c_coloring states)
  "the same function as above, only difference its this function only genereate
one possible coloring and then terminate."
  (if (not exit_flag)
      (let ((n (first t_sorted)))
	(if (null t_sorted)
	      (setf exit_flag (print (append c_coloring t_coloring)))
	      ;;(if (not (check-legal-assign states (append c_coloring t_coloring)))
	      ;;    (print "error!"))
	      (let ((parent_color (second (assoc (second n) t_coloring))))
		(dolist (x (remove parent_color (second (assoc (first n) domains))))
...
(defun gen-colorings-aux-not-used (domains c_states states colors t_sorted)
...
(defun run ()
  "the wrapper function, run the colorings"
  (defvar colors)
  (setf colors '(R G B Y))

  (print "*********************** Coloring United States *************************")

  (defvar *50-states*)
  (load "states.cl")

  (print "Vertices in the cutset:")
  (print (greedy-algorithm *50-states*))

  (setf exit_flag nil)
  (print "One possible coloring:")
  (gen-colorings *50-states* colors)

  
  (print "*********************** Coloring The World Map *************************")

  (load "countriesmap.lisp")

  (print "Vertices in the cutset:")
  (print (greedy-algorithm countries-map))
